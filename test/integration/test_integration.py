...
def test_analytical_example_2(micro_mesh, macro_mesh, eps):
    ...
    def A(x):
        ...
        return A
    def f(x):
        return 3.25696945235949*ufl.sqrt((0.454545454545455*ufl.sin(2*ufl.pi*x[0]) + 1)**2 - 0.206611570247934)*ufl.sin(ufl.pi*x[0])*ufl.sin(ufl.pi*x[1]) + ufl.pi**2*(0.15*ufl.sin(2*ufl.pi*x[0]) + 0.33)*ufl.sin(ufl.pi*x[0])*ufl.sin(ufl.pi*x[1]) - 2.96088132032681*(0.454545454545455*ufl.sin(2*ufl.pi*x[0]) + 1)*ufl.sin(ufl.pi*x[1])*ufl.cos(ufl.pi*x[0])*ufl.cos(2*ufl.pi*x[0])/ufl.sqrt((0.454545454545455*ufl.sin(2*ufl.pi*x[0]) + 1)**2 - 0.206611570247934)
